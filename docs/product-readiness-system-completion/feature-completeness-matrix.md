...
| Email | Auth messages | `PRODUCTION_DEPLOYED_PENDING_MANUAL_ACCEPTANCE` | `auth.email_verification` and resend are linked; the deployed OTP template is concise, branded, uses relative validity text, and avoids raw expiry timestamps or repeated product-name filler. QA delivery/manual acceptance remains. |
