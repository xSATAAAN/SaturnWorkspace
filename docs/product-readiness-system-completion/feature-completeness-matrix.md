...
| Auth | OTP request/queue/hash/purge contract | `PRODUCTION_DEPLOYED_PENDING_MANUAL_ACCEPTANCE` | Current production source makes email/password signup provider-only until OTP, stores only hashed OTP plus non-sensitive registration metadata, and finalizes the same Firebase UID profile on OTP. Auth/Site checks pass; Auth Worker version `a7caa6d5-e7f1-41ef-9974-c474a8a75f7f` and Pages run `28165825916` are deployed; provider inbox acceptance still needs QA recipient. |
