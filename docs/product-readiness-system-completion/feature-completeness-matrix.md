...
| --- | --- | --- | --- |
| Auth | Shared bootstrap and verified Saturn account identity | `PENDING_DEPLOYMENT_VERIFICATION` | Current source gates unfinalized email/password users before protected portal rendering and uses profile source for display name. Protected Saturn access now requires valid Firebase token, finalized Saturn custom claim, canonical finalized `account_profiles` row, UID agreement, lifecycle allowance, and fresh token auth time after credential reconciliation. Auth checks pass. The least-privilege Firebase finalizer identity, Auth Worker `FIREBASE_SERVICE_ACCOUNT_JSON`, and `FIREBASE_PROJECT_ID=saturnws-1` are configured; deployment and disposable QA acceptance remain pending. |
| Auth | OTP request/queue/hash/purge contract | `PENDING_DEPLOYMENT_VERIFICATION` | Current source starts a pending registration without a Saturn-created Firebase identity, stores only hashed OTP plus non-sensitive registration metadata, uses a one-time finalization token after OTP, fails closed if `FIREBASE_PROJECT_ID` does not match the service-account JSON `project_id`, and sets the finalized account claim only server-side after profile creation/reconciliation. Provider inbox acceptance still needs QA recipient and live canaries after deployment. |
...
| Admin security | UID role assignments and backend permissions | `PENDING_DEPLOYMENT_VERIFICATION` | Code supports `super_admin`, `support`, `billing`, `release_manager`, `security_auditor`, and `read_only`; UID-based `ADMIN_ROLE_ASSIGNMENTS` is configured as an Admin Worker secret for the currently authorized administrator. Authenticated route sweep remains pending. |
