...
| --- | --- | --- | --- |
| Auth | Shared bootstrap and verified Firebase UID identity | `PRODUCTION_DEPLOYED_PENDING_MANUAL_ACCEPTANCE` | Current source gates unverified email/password users before protected portal rendering and uses profile source for display name. Auth/Site checks pass; Auth Worker version `0186ad21-4c7b-4399-8c92-20a876fd5bee` and Pages run `28174200979` are deployed. Disposable QA acceptance remains Phase G. |
| Auth | OTP request/queue/hash/purge contract | `PRODUCTION_DEPLOYED_PENDING_MANUAL_ACCEPTANCE` | Current production source makes email/password signup provider-only until OTP, stores only hashed OTP plus non-sensitive registration metadata, finalizes the same Firebase UID profile on OTP, and enqueues delivery through the Policy service binding. Auth/Site checks pass; Auth Worker version `0186ad21-4c7b-4399-8c92-20a876fd5bee` and Pages run `28174200979` are deployed; provider inbox acceptance still needs QA recipient. |
| Auth | Pending-registration verification UI and Change email lifecycle | `PRODUCTION_DEPLOYED_PENDING_MANUAL_ACCEPTANCE` | Verification page uses structured pending-registration context, displays the destination email once without an editable email input, fails direct route safely, and calls server-side cancellation before returning to signup. Auth behavior tests prove old superseded codes cannot be reused. |
