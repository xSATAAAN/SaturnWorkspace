...
| PR-016 | Email/password signup bypassed Saturn OTP boundary | Current source verified, deployment pending | Root cause: frontend email signup provisioned profile immediately, Auth Worker account/bootstrap/device paths auto-provisioned unverified password identities, and the frontend email verification flag defaulted off when unset. Current source now requires OTP before any Saturn profile/protected portal/Desktop session for password identities, blocks explicit provision before OTP, finalizes profile on OTP, sets a server-controlled finalized custom claim, requires finalized profile plus claim on protected boundaries, quarantines stale raw provider-only password identities, and does not ship OTP test-code display/storage logic in the production bundle. Local Auth/Site checks and the direct-signup quarantine canary pass; least-privilege finalizer credentials are configured; deployment and disposable QA inbox acceptance remain pending. | Phase G |
