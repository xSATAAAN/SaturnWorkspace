...
- Optional OTA visibility now uses semantic prerelease comparison. Production beta currently publishes `1.1.2-beta`; a Desktop already running `1.1.2-beta` correctly reports no newer update, while `1.1.1-beta` resolves the same manifest as an available optional update. Admin displays the selected channel projection instead of the legacy stable root, keeps the entered release version when a differently named artifact is selected, and explains that only older clients receive the release.
