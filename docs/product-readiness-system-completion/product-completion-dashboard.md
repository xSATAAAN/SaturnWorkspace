...
- Email/password registration OTP boundary defect was found and fixed in current source: signup no longer provisions a Saturn profile before OTP, Auth account endpoints, explicit provision, and Desktop device linking now fail closed for unverified password identities, OTP verification finalizes the same Firebase UID profile with `verification_source = saturnws_otp`, the frontend verification gate defaults on, and the production site bundle no longer contains OTP test-code display/storage logic. Local Auth Worker and Site checks pass. Deployment and disposable QA inbox acceptance are still pending for this remediation batch.
...
| Email/password OTP remediation deployment | `PENDING_DEPLOYMENT_VERIFICATION` | Deploy Auth Worker and site after this source batch, then verify live registration with disposable QA without exposing password, OTP, UID, token, or cookies. |
