...
- OTA version contract: `PRODUCTION_DEPLOYED_PENDING_MANUAL_ACCEPTANCE`. The Admin Worker rejects installed update ZIP names outside `SaturnWorkspace-app-<version>.zip`, artifact/version mismatch, and normal publication of a version equal to or older than the active channel. Desktop comparison distinguishes prereleases from final releases. Admin Worker version `29d049f9-4a8d-4cc4-b792-3e741fd7f40b` is deployed.
...
- Optional OTA visibility now uses semantic prerelease comparison. Production beta currently publishes `1.1.2-beta`; a Desktop already running `1.1.2-beta` correctly reports no newer update, while `1.1.1-beta` resolves the same manifest as an available optional update. Admin now displays the selected channel projection instead of the legacy stable root, validates that metadata and the generated artifact filename agree, and explains that only older clients receive the release.
...
| QA Desktop Setup artifact | `QA_ARTIFACT_BUILT_PENDING_MANUAL_ACCEPTANCE` | Local artifact: `D:\SaturnWS\desktop-app\qa-builds\1.1.3-beta-20260714-ota-channel\setup\SaturnWorkspace-Setup-1.1.3-beta.exe`; size `41,486,146` bytes; SHA256 `98520095816179071ADBFE8B8C6F82ABD4F314490F656EAE19781E81BCFA643A`. Source/package payload hashes match and the isolated packaged runtime remained healthy for the full observation window. It was not installed over the active `1.1.2-beta` workstation so OTA can be tested from an older client. |
| Desktop source reproducibility | `RECORDED_PENDING_MANUAL_ACCEPTANCE` | `D:\SaturnWS\desktop-app` is not a Git repository. Current source/package parity covers `105/105` runtime payload files with aggregate SHA256 `66A7792F0E99B23CAA7DE88215AF357361F42539CAEFC0D4E58A72517EFA5A0F`. |
| Prepared Desktop OTA artifact | `PREPARED_NOT_PUBLISHED` | `D:\SaturnWS\desktop-app\qa-builds\1.1.3-beta-20260714-ota-channel\updates\SaturnWorkspace-app-1.1.3-beta.zip`; size `44,951,018` bytes; SHA256 `FC296FD21D3578F3B66E8ED6A5A1CB77D2F42B312982466DBE908D1E6BCA0DC5`. Internal version/build ID are `1.1.3-beta` / `2026-07-14-ota-channel-consistency`; production beta remains `1.1.2-beta` until an explicit later publication. |
...
- Production beta `1.1.2-beta` was published by the administrator as an optional update during live acceptance; `1.1.3-beta` remains prepared locally and unpublished.
