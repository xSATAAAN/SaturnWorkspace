...
export const EMAIL_CATALOG: Record<string, EmailCatalogItem> = {
  ...
  "admin.auth_orphan_quarantine": item({
    event_type: "admin.auth_orphan_quarantine",
    template_key: "admin_auth_orphan_quarantine",
    category: "admin",
    sender_identity: "general",
    title_en: "Auth orphan identities need review",
    title_ar: "Auth orphan identities need review",
    description_en: "Alerts administrators when password-only Firebase identities without Saturn finalization are observed or quarantined.",
    description_ar: "Alerts administrators when password-only Firebase identities without Saturn finalization are observed or quarantined.",
    default_subject_en: "SaturnWS auth orphan quarantine needs review",
    default_subject_ar: "SaturnWS auth orphan quarantine needs review",
    integration_status: "linked",
    user_can_disable: false,
    retry_allowed: true,
    essential: true,
