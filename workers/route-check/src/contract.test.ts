...
test("worker protocol matches the canonical desktop control-plane contract", () => {
  ...
  assert.equal(contract.route_check.operations.browser_acknowledgement.effect, "confirm_browser_processed_decision")
  assert.equal(contract.route_check.qualification.requires_browser_acknowledgement_before_targets, true)
