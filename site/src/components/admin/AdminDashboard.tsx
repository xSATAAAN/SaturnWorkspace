...
export function AdminDashboard({ lang }: AdminDashboardProps) {
  ...
  const handlePublishOta = async () => {
    ...
    const artifactVersion = releaseVersionFromArtifactName(otaInstalledFile.name)
    if (!artifactVersion) {
      setError('Use the installed update ZIP generated by the Saturn Workspace build pipeline.')
      return
    }
    if (artifactVersion.toLowerCase() !== otaVersion.trim().toLowerCase()) {
      setError(`The entered version must match the installed ZIP version (${artifactVersion}).`)
      return
    }
    setSaving(true)
    setError(null)
    try {
      ...
      setError(
        message === 'same_version_already_active'
          ...
          : message === 'release_version_filename_mismatch'
            ? 'The release version does not match the version in the artifact filename.'
          : message === 'release_artifact_filename_invalid'
            ? 'Use the installed update ZIP generated by the Saturn Workspace build pipeline.'
          : message,
      )
    } finally {
      setSaving(false)
                  ...
                  <label className="grid gap-1 text-xs text-white/65">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        setOtaInstalledFile(file)
                        const artifactVersion = releaseVersionFromArtifactName(file?.name || '')
                        if (artifactVersion) setOtaVersion(artifactVersion)
                      }}
                    />
                  </label>
                  <label className="grid gap-1 text-xs text-white/65">
