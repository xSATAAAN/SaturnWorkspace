...
export function PricingCard({ name, description, price, period, features, cta, featured, disabled, featuredLabel, onClick }: { name: string; description: string; price: string; period?: string; features: string[]; cta: string; featured?: boolean; disabled?: boolean; featuredLabel?: string; onClick?: () => void }) {
  return <Card className={`pricing-card${featured ? ' is-featured' : ''}`}><div className="pricing-card__head"><h3>{name}</h3>{featured && featuredLabel ? <Badge tone="info">{featuredLabel}</Badge> : null}</div><p>{description}</p><div className="pricing-card__price"><strong>{price}</strong>{period ? <span>{period}</span> : null}</div><ul>{features.map((feature) => <li key={feature}><Check size={15} />{feature}</li>)}</ul><Button variant={featured ? 'primary' : 'secondary'} fullWidth disabled={disabled} onClick={onClick}>{cta}</Button></Card>
}
