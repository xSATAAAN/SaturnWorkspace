...
function AdminReleases() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (file) {
      const artifactVersion = releaseVersionFromArtifactName(file.name, artifactType)
      if (artifactType === 'installed' && !artifactVersion) {
        setError(copyByLocale(
          locale,
          'Use the installed update ZIP generated by the build pipeline.',
          'استخدم ملف ZIP الخاص بالتحديث المثبت والناتج من مسار البناء.',
        ))
        return
      }
      if (artifactVersion && artifactVersion.toLowerCase() !== version.trim().toLowerCase()) {
        setError(copyByLocale(
          locale,
          `The entered version must match the artifact version (${artifactVersion}).`,
          `يجب أن يطابق الإصدار المكتوب إصدار الملف (${artifactVersion}).`,
        ))
        return
      }
    }
