...
function PricingPreview({ navigate, full = false }: { navigate: Navigate; full?: boolean }) {
  ...
  const c = publicCopy[locale]
  const plans = [
    { name: c.weeklyName, description: c.weeklyBody, price: c.weeklyPrice, period: c.weeklyPeriod, features: [c.featureWorkspace, c.featureProfiles, c.featureUpdates], featured: false },
    { name: c.monthlyName, description: c.monthlyBody, price: c.monthlyPrice, period: c.monthlyPeriod, features: [c.featureWorkspace, c.featureProfiles, c.featureRecovery], featured: false },
    { name: c.annualName, description: c.annualBody, price: c.annualPrice, period: c.annualPeriod, features: [c.featureWorkspace, c.featureProfiles, c.featureRecovery, c.featureUpdates], featured: true },
  ]
  return <section className={`marketing-section pricing-section${full ? ' pricing-page' : ''}`}><div className="container"><Reveal><header className="marketing-heading marketing-heading--center">{full ? <h1>{c.pricingTitle}</h1> : <h2>{c.pricingTitle}</h2>}<p>{c.pricingBody}</p></header></Reveal><div className="pricing-grid">{plans.map((plan, index) => <Reveal key={plan.name} delay={index * 80}><PricingCard name={plan.name} description={plan.description} price={plan.price} period={plan.period} features={plan.features} cta={c.requestAccess} featured={plan.featured} featuredLabel={c.recommended} onClick={() => navigate({ surface: 'auth', page: 'signup' })} /></Reveal>)}</div>{full ? <PlanComparison /> : <div className="marketing-center"><Button variant="text" trailingIcon={<ArrowRight size={15} />} onClick={() => navigate({ surface: 'public', page: 'pricing' })}>{locale === 'ar' ? 'عرض تفاصيل الاشتراك' : 'View plan details'}</Button></div>}</div></section>
}
