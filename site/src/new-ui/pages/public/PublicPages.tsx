...
function PricingPreview({ navigate, full = false }: { navigate: Navigate; full?: boolean }) {
  ...
  const c = publicCopy[locale]
  const plans = [
    { name: c.weeklyName, price: c.weeklyPrice, originalPrice: c.weeklyOriginalPrice, period: c.weeklyPeriod, featured: false, note: '' },
    { name: c.monthlyName, price: c.monthlyPrice, originalPrice: c.monthlyOriginalPrice, period: c.monthlyPeriod, featured: true, note: locale === 'ar' ? '7 أيام تجربة مجانية' : '7 days free trial' },
    { name: c.annualName, price: c.annualPrice, originalPrice: c.annualOriginalPrice, period: c.annualPeriod, featured: false, note: locale === 'ar' ? '7 أيام تجربة مجانية' : '7 days free trial' },
  ]
  return <section className={`marketing-section pricing-section${full ? ' pricing-page' : ''}`}><div className="container"><Reveal><header className="marketing-heading marketing-heading--center marketing-heading--wide">{full ? <h1>{c.pricingTitle}</h1> : <h2>{c.pricingTitle}</h2>}<p>{c.pricingBody}</p></header></Reveal><div className="pricing-grid">{plans.map((plan, index) => <Reveal key={plan.name} delay={index * 80}><PricingCard name={plan.name} price={plan.price} originalPrice={plan.originalPrice} period={plan.period} note={plan.note} features={[]} cta={c.requestAccess} featured={plan.featured} featuredLabel={c.recommended} onClick={() => navigate({ surface: 'auth', page: 'signup' })} /></Reveal>)}</div>{full ? null : <div className="marketing-center"><Button variant="text" trailingIcon={<ArrowRight size={15} />} onClick={() => navigate({ surface: 'public', page: 'pricing' })}>{locale === 'ar' ? 'عرض تفاصيل الاشتراك' : 'View plan details'}</Button></div>}</div></section>
}
