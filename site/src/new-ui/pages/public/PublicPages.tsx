...
function PricingPreview({ navigate, full = false }: { navigate: Navigate; full?: boolean }) {
  ...
  const plans = [
    { name: c.weeklyName, description: c.weeklyBody, price: c.weeklyPrice, originalPrice: c.weeklyOriginalPrice, period: c.weeklyPeriod, features: [locale === 'ar' ? 'مرونة أسبوعية بدون التزام طويل' : 'Weekly flexibility without a long commitment'], savingsLabel: locale === 'ar' ? 'خصم حالي من 15 إلى 10 دولار' : 'Current discount from $15 to $10', trialLabel: undefined, featured: false },
    { name: c.monthlyName, description: c.monthlyBody, price: c.monthlyPrice, originalPrice: c.monthlyOriginalPrice, period: c.monthlyPeriod, features: [locale === 'ar' ? 'مناسب للاستخدام المنتظم' : 'Built for regular use'], savingsLabel: locale === 'ar' ? 'خصم حالي من 50 إلى 35 دولار' : 'Current discount from $50 to $35', trialLabel: locale === 'ar' ? '7 أيام تجربة مجانية قبل أول تحصيل' : '7 free trial days before first charge', featured: true },
    { name: c.annualName, description: c.annualBody, price: c.annualPrice, originalPrice: c.annualOriginalPrice, period: c.annualPeriod, features: [locale === 'ar' ? 'أكبر توفير على المدى الطويل' : 'Largest long-term saving'], savingsLabel: locale === 'ar' ? 'خصم حالي من 600 إلى 350 دولار' : 'Current discount from $600 to $350', trialLabel: locale === 'ar' ? '7 أيام تجربة مجانية قبل أول تحصيل' : '7 free trial days before first charge', featured: false },
  ]
  return <section className={`marketing-section pricing-section${full ? ' pricing-page' : ''}`}><div className="container"><Reveal><header className="marketing-heading marketing-heading--center marketing-heading--wide">{full ? <h1>{c.pricingTitle}</h1> : <h2>{c.pricingTitle}</h2>}<p>{c.pricingBody}</p></header></Reveal><div className="pricing-promo">{c.trialPromo}</div><div className="pricing-included"><strong>{c.compareTitle}</strong>{sharedBenefits.map((feature) => <span key={feature}><Check size={14} />{feature}</span>)}</div><div className="pricing-grid">{plans.map((plan, index) => <Reveal key={plan.name} delay={index * 80}><PricingCard name={plan.name} description={plan.description} price={plan.price} originalPrice={plan.originalPrice} period={plan.period} features={plan.features} cta={c.requestAccess} featured={plan.featured} featuredLabel={c.recommended} trialLabel={plan.trialLabel} savingsLabel={plan.savingsLabel} onClick={() => navigate({ surface: 'auth', page: 'signup' })} /></Reveal>)}</div>{full ? <PlanComparison /> : <div className="marketing-center"><Button variant="text" trailingIcon={<ArrowRight size={15} />} onClick={() => navigate({ surface: 'public', page: 'pricing' })}>{locale === 'ar' ? 'عرض تفاصيل الاشتراك' : 'View plan details'}</Button></div>}</div></section>
}
