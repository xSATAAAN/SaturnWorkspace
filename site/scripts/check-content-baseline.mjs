...
const checks = [
  ...
  {
    name: 'copy removes implementation-facing and redundant state narration',
...
if (failed.length) {
  console.error('Content baseline checks failed:')
  failed.forEach((check) => console.error(`- ${check.name}`))
  process.exit(1)
}

console.log('Content baseline checks passed.')
