...
function readMigrationContaining(relativeDirectory, requiredTokens) {
  const directory = path.join(WEB_PLATFORM, relativeDirectory)
  ...
  throw new Error(`migration contract missing in ${relativeDirectory}: ${requiredTokens.join(', ')}`)
}
...
const adminOperations = read('site/src/new-ui/pages/production/AdminOperations.tsx')
...
assertIncludes(adminOperations, 'admin.updateGlobalPolicy', 'admin policy update UI')
...
if (missingBackendFiles.length) {
  throw new Error(`Backend contract source is missing: ${missingBackendFiles.join(', ')}`)
}

{
  ...
  const commercialMigration = readMigrationContaining(
    'workers/auth/migrations',
    ['commercial_plans', 'commercial_orders'],
  )
  ...
  assertIncludes(commercialMigration, 'commercial_plans', 'commercial migration plans table')
  assertIncludes(commercialMigration, 'commercial_orders', 'commercial migration orders table')
...
console.log('Production integration contract checks passed.')
